...
4. 文档内容抓取：query_type = "fetch"
   ...
   - 或提供 keyword 参数：自动在本地索引中找最匹配的文档并抓取（一步完成，无需先 search）
   - 返回文档正文、代码块、表格、警告框等结构化内容
   - 页面内容缓存 15 分钟

**重要提示：**
- 用户想查看文档具体内容时，直接使用 fetch + keyword，工具会自动从本地索引定位文档并抓取，不需要先调 search 再调 fetch
- 本工具内部优先检索本地 llms.txt 索引，无结果时才自动调用远程搜索 API 兜底，无需外部额外调用搜索接口
- 本工具仅用于查询文档内容，不用于查询可安装的软件包/应用信息（软件包信息请用 get_app_info）
...
**不适用场景（请勿使用本工具）：**
- 查询可安装的软件包、开发工具、社区组件 → 请用 get_app_info
- 查询软件包的版本、下载地址、安装方法 → 请用 get_app_info

**示例问题：**
- "openUBMC 文档中心有哪些章节？"
- "查找关于 BMC Studio 的文档"
- "openUBMC 如何构建 BMC？"（→ 直接用 fetch+keyword）
- "快速入门章节有哪些文档？"
- "ORM 怎么使用？查看文档内容"
- "CSR配置字典有哪些类？"`,
  inputSchema: {
