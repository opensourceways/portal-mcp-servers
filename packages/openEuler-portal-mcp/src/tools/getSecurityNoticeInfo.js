...
// 查询安全公告列表（带缓存）
async function fetchNoticeList(keyword = "") {
  ...
  const response = await fetch(NOTICE_LIST_URL, {
    ...
    body: JSON.stringify({ pages: { page: 1, size: PAGE_SIZE }, keyword, noticeType: "cve" }),
...
// 查询安全公告详情（带缓存）
async function fetchNoticeDetail(securityNoticeNo) {
  const now = Date.now();
  ...
  if (data.code !== 0 || !data.result) {
    throw new Error(`安全公告不存在或查询失败：${securityNoticeNo}`);
  }
...
// 解析 CVE ID 字符串（末尾分号清理）
function parseCveIds(cveId) {
  if (!cveId) return [];
...
function formatNoticeItem(notice, index) {
  ...
  const cveIds = parseCveIds(notice.cveId);

  let output = `${index + 1}. **${notice.securityNoticeNo}**\n`;
  output += `   摘要: ${notice.summary || "暂无"}\n`;
  output += `   危险等级: ${severity}\n`;
  output += `   受影响组件: ${notice.affectedComponent || "未知"}\n`;
  output += `   受影响版本: ${notice.affectedProduct || "未知"}\n`;

  if (cveIds.length > 0) {
    const showIds = cveIds.slice(0, 3).join(", ");
    const more = cveIds.length > 3 ? ` 等共 ${cveIds.length} 个` : "";
    output += `   CVE: ${showIds}${more}\n`;
  }
...
function formatNoticeDetail(notice) {
  ...
  if (cveIds.length > 0) {
    output += `\n**关联 CVE（${cveIds.length} 个）:**\n`;
    cveIds.forEach(id => { output += `   - ${id}\n`; });
  }
  ...
  output += `\n---\n`;
  output += `数据来源: openEuler CVE 安全公告系统\n`;
...
// 主查询函数
export async function getSecurityNoticeInfo(queryType = "list", keyword = "", securityNoticeNo = "") {
  try {
    if (queryType === "detail") {
      if (!securityNoticeNo) {
        return "请提供安全公告编号，格式如：openEuler-SA-2026-1486";
      }
      const notice = await fetchNoticeDetail(securityNoticeNo);
      return formatNoticeDetail(notice);
    }

    // 默认 list 查询
    const result = await fetchNoticeList(keyword);
    ...
    if (notices.length === 0) {
      let output = `=== openEuler 安全公告查询 ===\n\n查询条件：${conditions}\n\n`;
      output += `未找到符合条件的安全公告。\n`;
      if (keyword) {
        output += `\n💡 提示：可以尝试更换关键词，例如组件名（kernel、openssl）或公告编号。\n`;
      }
      return output;
    }
    ...
    output += `共找到 ${total} 条安全公告，显示最新 ${displayNotices.length} 条\n\n`;
    output += `---\n\n`;
    ...
    if (total > 20) {
      output += `⚠️ 共 ${total} 条公告，仅显示最新 20 条。建议添加关键词（如组件名、CVE 编号）缩小范围。\n\n`;
    }
    ...
    return `查询安全公告时发生错误：${e.message}`;
  }
}

// 工具定义
export const toolDefinition = {
  name: "get_security_notice_info",
  description: `查询 openEuler 安全公告（Security Notice）信息，支持列表查询和公告详情查询。

本工具用于查询 openEuler 发布的安全公告，每条公告对应一批 CVE 安全漏洞的修复情况。

**使用场景：**
- 查询某个组件（如 kernel、openssl）相关的安全公告
- 按关键词搜索安全公告
- 查看某条安全公告的详细信息（漏洞描述、修复包、参考链接）
- 了解 openEuler 最新发布的安全更新
...
2. 详情查询：query_type = "detail"
   - 查询指定安全公告的完整内容
   - 需提供 security_notice_no（公告编号，如 openEuler-SA-2026-1486）
   - 返回漏洞描述、关联 CVE、修复软件包、参考链接等完整信息

**参数说明：**
- query_type: 查询类型，"list"（列表，默认）或 "detail"（详情）
- keyword: 搜索关键词（可选），如组件名（kernel、openssl）或 CVE 编号；query_type 为 "list" 时有效
- security_notice_no: 安全公告编号，如 openEuler-SA-2026-1486；query_type 为 "detail" 时必填

**返回信息（列表）：**
- 公告编号（securityNoticeNo）
- 摘要
...
- "CVE-2026-2239 对应的安全公告详情是什么？"`,
  inputSchema: {
    ...
    properties: {
      ...
      keyword: {
        type: "string",
        description: "搜索关键词（可选），如组件名（kernel、openssl、glibc）或 CVE 编号。query_type 为 'list' 时有效，为空时返回全部最新公告。",
        default: ""
      },
      security_notice_no: {
        type: "string",
        description: "安全公告编号，格式如 openEuler-SA-2026-1486。query_type 为 'detail' 时必填。",
        default: ""
      }
    }
  }
